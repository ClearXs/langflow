    ...
    <>
      ...
      >
        ...
        <BaseModal.Header
          description={
            <span className="pr-2">
              {t("api.modal.description.requiresKey")}{" "}
              <CustomLink
                to="/settings/api-keys"
                className="text-accent-pink-foreground"
              >
                {t("api.modal.description.createKey")}
              </CustomLink>{" "}
              {t("api.modal.description.inSettings")}
            </span>
          }
        >
          <IconComponent
            ...
            aria-hidden="true"
          />
          <span className="pl-2">{t("api.modal.title")}</span>
          {nodes.length > 0 && (
            <div className="border-r-1 absolute right-12 flex items-center text-mmd font-medium leading-[16px]">
              ...
              >
                ...
                />
                <span>
                  {t("api.modal.inputSchema")} ({Object.keys(tweaks)?.length})
                </span>
              </Button>
      ...
      >
        <BaseModal.Header>
          ...
          <span className="pl-2">{t("api.modal.inputSchema")}</span>
        </BaseModal.Header>
            ...
            <Label>
              <div className="edit-flow-arrangement mt-2">
                <span className="shrink-0 text-mmd font-medium">
                  {t("api.modal.endpointName.label")}
                </span>
                {!validEndpointName && (
              ...
              <Input
                ...
                name="endpoint_name"
                value={endpointName ?? ""}
                placeholder={t("api.modal.endpointName.placeholder")}
          ...
          <div className="flex flex-1 flex-col gap-2 overflow-hidden">
            <div className="flex flex-col gap-1">
              <span className="shrink-0 text-sm font-medium">
                {t("api.modal.exposeApi.title")}
              </span>
              <span className="text-mmd text-muted-foreground">
                {t("api.modal.exposeApi.description")}
              </span>
            </div>
