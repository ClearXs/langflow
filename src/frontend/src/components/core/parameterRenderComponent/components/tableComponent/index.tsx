...
>(({ displayEmptyAlert = true, ...props }, ref) => {
  ...
  const {
    alertTitle = messageLocale.DEFAULT_TABLE_ALERT_TITLE,
    alertDescription = messageLocale.DEFAULT_TABLE_ALERT_MSG,
  } = props;
      ...
      <div className='flex h-full w-full items-center justify-center rounded-md border'>
        <Alert variant={'default'} className='w-fit'>
          <ForwardedIconComponent
            ...
            className='h-5 w-5 text-primary'
          />
          <AlertTitle>{messageLocale.NO_COLUMN_DEFINITION_ALERT_TITLE}</AlertTitle>
          <AlertDescription>
