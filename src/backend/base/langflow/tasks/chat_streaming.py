...
- LLMConfig database table (positive IDs for user-created configs with prompt settings)
"""
...
from langflow.agents.new_chat.chat_deepagent import create_surfsense_deep_agent
from langflow.agents.new_chat.checkpointer import get_checkpointer
from langflow.agents.new_chat.llm_config import (
    AgentConfig,
...
async def stream_new_chat_response(
    user_query: str,
    search_space_id: int,
    chat_id: int,
    ...
    try:
        ...
        if llm_config_id >= 0:
            ...
            if not agent_config:
                yield streaming_service.format_error(
                    f"Failed to load LLMConfig with id {llm_config_id}"
                )
        ...
        print(f"[stream_new_chat_response] {error_message}")
