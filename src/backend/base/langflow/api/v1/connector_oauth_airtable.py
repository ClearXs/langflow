...
async def connect_airtable(space_id: int, current_user: CurrentActiveUser):
    ...
    try:
        ...
        if not settings.AIRTABLE_CLIENT_ID:
            raise HTTPException(
        ...
        logger.info(
            f"Generated Airtable OAuth URL for user {current_user.id}, space {space_id}"
        )
...
async def airtable_callback(
    ...
    try:
        ...
        auth_header = make_basic_auth_header(
            settings.AIRTABLE_CLIENT_ID, settings.AIRTABLE_CLIENT_SECRET
        )
        ...
        existing_connector_result = await db.execute(
            select(Connector).filter(
                Connector.search_space_id == space_id,
                Connector.user_id == user_id,
                Connector.connector_type
                == ConnectorType.AIRTABLE_CONNECTOR,
            )
        )
        ...
        if existing_connector:
            ...
            new_connector = Connector(
                ...
                connector_type=ConnectorType.AIRTABLE_CONNECTOR,
                is_indexable=True,
        ...
        try:
            ...
            return RedirectResponse(
                url=f"{settings.NEXT_FRONTEND_URL}/dashboard/{space_id}/connectors/add/airtable-connector?success=true"
            )
