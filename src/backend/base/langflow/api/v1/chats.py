...
@router.get("/threads/{thread_id}/full", response_model=ChatThreadWithMessages)
...
@router.put("/threads/{thread_id}", response_model=ChatThreadRead)
async def update_thread(
    thread_id: int,
    thread_update: ChatThreadUpdate,
    db: DbSession = None,
    current_user: CurrentActiveUser = None,
):
...
async def append_message(
    ...
    try:
        ...
        message = ChatMessageAppend(role=role, content=content)
        # Get thread
        result = await db.execute(
            select(ChatThread).filter(ChatThread.id == thread_id)
        )
        ...
        try:
            message_role = ChatMessageRole(role_str)
        except ValueError:
            raise HTTPException(
                status_code=400,
...
async def handle_new_chat(
    ...
    try:
        ...
        result = await db.execute(
            select(ChatThread).filter(ChatThread.id == request.chat_id)
        )
        thread = result.scalars().first()
    ...
    try:
        ...
        if file_ext in (".md", ".markdown", ".txt"):
            ...
            if stt_service_type == "local":
                from langflow.services.stt import stt_service
