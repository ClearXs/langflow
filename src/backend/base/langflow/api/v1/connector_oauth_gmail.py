...
async def connect_gmail(space_id: int, current_user: CurrentActiveUser):
    try:
        if not space_id:
        ...
        try:
            ...
            result = await db.execute(
                select(Connector).filter(
                    ...
                    == ConnectorType.GOOGLE_GMAIL_CONNECTOR,
                )
            )
            ...
            db_connector = Connector(
                ...
                connector_type=ConnectorType.GOOGLE_GMAIL_CONNECTOR,
                config=creds_dict,
            ...
            return RedirectResponse(
                url=f"{settings.NEXT_FRONTEND_URL}/dashboard/{space_id}/connectors/add/google-gmail-connector?success=true"
            )
