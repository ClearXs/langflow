    ...
    try:
        ...
        if event_delivery in (EventDeliveryType.STREAMING, EventDeliveryType.DIRECT):
            if event_task is None:
                ...
                raise HTTPException(status_code=404, detail=i18n.t("api.errors.no_event_task_found"))
        ...
        try:
            ...
            raise HTTPException(status_code=499, detail=i18n.t("api.errors.event_polling_cancelled")) from exc
        ...
        raise HTTPException(status_code=404, detail=i18n.t("api.errors.job_not_found", error=str(exc))) from exc
    except Exception as exc:
        if isinstance(exc, HTTPException):
            raise
        await logger.aexception(f"Unexpected error processing flow events for job {job_id}")
        raise HTTPException(status_code=500, detail=i18n.t("api.errors.unexpected_error", error=str(exc))) from exc
