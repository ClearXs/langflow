"""BlockNote document format conversion utilities for Langflow backend."""
...
async def convert_markdown_to_blocknote(markdown: str) -> dict[str, Any] | None:
    ...
    # TODO: Add next_frontend_url to Settings configuration
    from langflow.services.deps import get_settings_service
    settings = get_settings_service().settings
    next_frontend_url = getattr(settings, "next_frontend_url", None)
    if not next_frontend_url:
        logger.error("next_frontend_url not configured in Settings")
        return None

    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{next_frontend_url}/api/convert-to-blocknote",
...
async def convert_blocknote_to_markdown(
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{next_frontend_url}/api/convert-to-markdown",
