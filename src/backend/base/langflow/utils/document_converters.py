"""Document conversion utilities for Langflow backend."""

import hashlib

from litellm import get_model_info, token_counter

from langflow.services.database.models import Chunk, DocumentType
...
async def generate_document_summary(
    ...
    summary_embedding = settings.embedding_model_instance.embed(enhanced_summary_content)
...
async def create_document_chunks(content: str) -> list[Chunk]:
    ...
    return [
        Chunk(
            content=chunk.text,
            embedding=settings.embedding_model_instance.embed(chunk.text),
        )
        for chunk in settings.chunker_instance.chunk(content)
    ]
