...
# Try to import firecrawl - fallback to trafilatura if not available
try:
...
class WebCrawlerConnector:
    ...
    async def _crawl_with_firecrawl(
        ...
        Raises:
            ValueError: If Firecrawl is not available or scraping fails
        """
        if not FIRECRAWL_AVAILABLE:
            logger.warning("Firecrawl is not available, falling back to Chromium crawler")
            return await self._crawl_with_chromium(url)
        ...
        # Run sync scrape in executor since FirecrawlApp doesn't support async in this version
        loop = asyncio.get_event_loop()
        scrape_result = await loop.run_in_executor(
            None,
            functools.partial(firecrawl_app.scrape, url, formats=formats)
        )
