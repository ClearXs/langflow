...
from langflow.services.database.models.connector import ConnectorType
...
async def index_github_repos(
    ...
    Args:
        ...
        space_id: ID of the space to store documents in
    ...
    try:
        ...
        connector = await get_connector_by_id(
            session, connector_id, ConnectorType.GITHUB
        )
            ...
            try:
                ...
                for file_info in files_to_index:
                    ...
                    unique_identifier_hash = generate_unique_identifier_hash(
                        DocumentType.GITHUB_CONNECTOR, file_sha, space_id
                    )

                    # Generate content hash
                    content_hash = generate_content_hash(file_content, space_id)
                    ...
                    try:
                        ...
                        if hasattr(settings, "code_chunker_instance"):
                            chunks_data = [
                                {
                                    "content": chunk.text,
                                    "embedding": settings.embedding_model_instance.embed(
                                        chunk.text
                                    ),
                                }
                                for chunk in settings.code_chunker_instance.chunk(
                                    file_content
                                )
                            ]
                        else:
