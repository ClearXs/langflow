...
2. Database LLMConfig table (user-created configs with positive IDs)
...
def load_llm_config_from_yaml(llm_config_id: int = -1) -> dict | None:
    ...
    config_file = base_dir / "config" / "global_llm_config.yaml"

    # Fallback to example file if main config doesn't exist
    if not config_file.exists():
        config_file = base_dir / "config" / "global_llm_config.example.yaml"
        if not config_file.exists():
...
async def load_llm_config_from_db(
    ...
    Args:
        ...
        config_id: The ID of the LLMConfig to load
    ...
    try:
        ...
        if not config:
            print(f"Error: LLMConfig with id {config_id} not found")
            return None

        return AgentConfig.from_llm_config(config)
    except Exception as e:
        print(f"Error loading LLMConfig from database: {e}")
        return None


async def load_agent_llm_config_for_space(
    session: AsyncSession,
    space_id: int,
) -> "AgentConfig | None":
    """
    ...
    try:
        # Get the space to check its agent_llm_id preference
        result = await session.execute(
        ...
        if not space:
            print(f"Error: Space with id {space_id} not found")
            return None

        # Use agent_llm_id from space, fallback to -1 (first global config)
        config_id = (
            space.agent_llm_id if space.agent_llm_id is not None else -1
        )

        # Load the config using the unified loader
        return await load_agent_config(session, config_id, space_id)
    except Exception as e:
        print(f"Error loading agent LLM config for space {space_id}: {e}")
        return None
...
async def load_agent_config(
    ...
    - Positive IDs: Load from LLMConfig database table
