...
- Only one podcast can be generated at a time per space
...
def get_active_podcast_key(space_id: int) -> str:
    """Generate Redis key for tracking active podcast task."""
    return f"podcast:active:{space_id}"


def get_active_podcast_task(space_id: int) -> str | None:
    """Check if there's an active podcast task for this space."""
    try:
        client = get_redis_client()
        return client.get(get_active_podcast_key(space_id))
    except Exception:
        # If Redis is unavailable, allow the request (fail open)
        return None


def set_active_podcast_task(space_id: int, task_id: str) -> None:
    """Mark a podcast task as active for this space."""
    try:
        client = get_redis_client()
        # Set with 30-minute expiry as safety net (podcast should complete before this)
        client.setex(get_active_podcast_key(space_id), 1800, task_id)
    except Exception as e:
        print(f"[generate_podcast] Warning: Could not set active task in Redis: {e}")


def clear_active_podcast_task(space_id: int) -> None:
    """Clear the active podcast task for this space."""
    try:
        client = get_redis_client()
        client.delete(get_active_podcast_key(space_id))
    except Exception as e:
...
def create_generate_podcast_tool(
    ...
    async def generate_podcast(
        ...
        """
        try:
            # Check if a podcast is already being generated for this space
            active_task_id = get_active_podcast_task(space_id)
            if active_task_id:
                print(
            ...
            from langflow.tasks.podcast_tasks import (
            ...
            set_active_podcast_task(space_id, task.id)
