...
from langflow.services.llm.service import get_document_summary_llm
...
async def create_podcast_transcript(
    ...
    # Get space's document summary LLM
    llm = await get_document_summary_llm(state.db_session, space_id)
    if not llm:
        error_message = (
            f"No document summary LLM configured for space {space_id}"
        )
        print(error_message)
...
async def create_merged_podcast_audio(
    ...
    async def generate_speech_for_segment(segment, index):
        ...
        voice = get_voice_for_provider(settings.tts_service, speaker_id)

        # Generate a unique filename for this segment
        if settings.tts_service == "local/kokoro":
        ...
        try:
            if settings.tts_service == "local/kokoro":
                # Use Kokoro TTS service (lazy import to avoid dependency issues)
                try:
                    from langflow.services.kokoro_tts.service import get_kokoro_tts_service
                    ...
                    raise RuntimeError(
                        "Kokoro TTS service is not available. "
                        "Please install the required dependencies or use a different TTS service."
                    )
            else:
                if settings.tts_service_api_base:
                    response = await aspeech(
                        model=settings.tts_service,
                        api_base=settings.tts_service_api_base,
                        api_key=settings.tts_service_api_key,
                        voice=voice,
                        input=dialog,
                        max_retries=2,
                        timeout=600,
