...
def generate_english_text(key_parts: list[str]) -> str:
    ...
    if "error" in last_part or last_part in ["file_not_found", "read_failed", "invalid_json", "unsupported_type"]:
        return f"Error: {title}"
    if last_part in ["reading", "success", "processing"]:
        return f"{title}..."
    return title
...
def main():
    ...
    categories = ["input_output", "operations", "manipulations", "security", "scripts"]
