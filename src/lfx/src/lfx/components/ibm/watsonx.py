...
class WatsonxAIComponent(LCModelComponent):
    ...
    inputs = [
        *LCModelComponent.get_base_inputs(),
        ...
        StrInput(
            name="project_id",
            display_name=i18n.t("components.ibm.watsonx.project_id.display_name"),
            required=True,
        ...
        DropdownInput(
            name="model_name",
            display_name=i18n.t("components.ibm.watsonx.model_name.display_name"),
            options=[],
            value=None,
            dynamic=True,
            required=True,
        ...
        IntInput(
            name="max_tokens",
            display_name=i18n.t("components.ibm.watsonx.max_tokens.display_name"),
            advanced=True,
        ...
        StrInput(
            name="stop_sequence",
            display_name=i18n.t("components.ibm.watsonx.stop_sequence.display_name"),
            advanced=True,
        ...
        SliderInput(
            name="temperature",
            display_name=i18n.t("components.ibm.watsonx.temperature.display_name"),
        ...
        SliderInput(
            ...
            display_name=i18n.t("components.ibm.watsonx.frequency_penalty.display_name"),
        ...
        SliderInput(
            ...
            display_name=i18n.t("components.ibm.watsonx.presence_penalty.display_name"),
        ...
        BoolInput(
            name="logprobs",
            display_name=i18n.t("components.ibm.watsonx.logprobs.display_name"),
            advanced=True,
        ...
        IntInput(
            name="top_logprobs",
            display_name=i18n.t("components.ibm.watsonx.top_logprobs.display_name"),
            advanced=True,
        ...
        StrInput(
            name="logit_bias",
            display_name=i18n.t("components.ibm.watsonx.logit_bias.display_name"),
            advanced=True,
    ...
    def fetch_models(base_url: str) -> list[str]:
        ...
        """
        logger.debug(i18n.t("components.ibm.watsonx.logs.fetching_models", base_url=base_url))

        try:
            endpoint = f"{base_url}/ml/v1/foundation_model_specs"
            params = {"version": "2024-09-16", "filters": "function_text_chat,!lifecycle_withdrawn"}

            logger.debug(i18n.t("components.ibm.watsonx.logs.requesting_models", endpoint=endpoint))
            ...
            logger.info(i18n.t("components.ibm.watsonx.logs.models_fetched", count=len(models)))

            return sorted(models)

        except Exception as e:
            error_msg = i18n.t("components.ibm.watsonx.errors.fetch_failed", error=str(e))
            logger.exception(error_msg)
            logger.info(
                i18n.t(
                    "components.ibm.watsonx.logs.using_default_models", count=len(WatsonxAIComponent._default_models)
                )
            )
    ...
    def update_build_config(self, build_config: dotdict, field_value: Any, field_name: str | None = None):
        ...
        logger.info(
            i18n.t(
                "components.ibm.watsonx.logs.updating_config",
        ...
        if field_name == "url" and field_value:
            try:
                logger.debug(i18n.t("components.ibm.watsonx.logs.fetching_models_for_url", url=build_config.url.value))
                ...
                info_message = i18n.t(
                    "components.ibm.watsonx.logs.models_updated", count=len(models), url=build_config.url.value
                )
                logger.info(info_message)

            except Exception as e:
                error_msg = i18n.t("components.ibm.watsonx.errors.config_update_failed", error=str(e))
                logger.exception(error_msg)
    ...
    def build_model(self) -> LanguageModel:
        ...
        """
        logger.info(i18n.t("components.ibm.watsonx.logs.building_model", model=self.model_name, url=self.url))

        # Parse logit_bias from JSON string if provided
        logit_bias = None
        if hasattr(self, "logit_bias") and self.logit_bias:
            try:
                logger.debug(i18n.t("components.ibm.watsonx.logs.parsing_logit_bias"))
                logit_bias = json.loads(self.logit_bias)
                logger.debug(i18n.t("components.ibm.watsonx.logs.logit_bias_parsed", bias=str(logit_bias)))
            except json.JSONDecodeError as e:
                error_msg = i18n.t("components.ibm.watsonx.errors.invalid_logit_bias", error=str(e))
                logger.warning(error_msg)
                logit_bias = {"1003": -100, "1004": -100}
                logger.info(i18n.t("components.ibm.watsonx.logs.using_default_logit_bias"))

        logger.debug(
            i18n.t(
                "components.ibm.watsonx.logs.configuring_parameters",
        ...
        logger.debug(i18n.t("components.ibm.watsonx.logs.creating_chat_instance"))
