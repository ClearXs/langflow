...
class ComposioAPIComponent(LCToolComponent):
    ...
    inputs = [
        ...
        MessageTextInput(
            name="entity_id",
            display_name=i18n.t("components.composio.composio_api.entity_id.display_name"),
            value="default",
            advanced=True,
        ),
        SecretStrInput(
            name="api_key",
            display_name=i18n.t("components.composio.composio_api.api_key.display_name"),
            required=True,
        ...
        ConnectionInput(
            name="tool_name",
            display_name=i18n.t("components.composio.composio_api.tool_name.display_name"),
            placeholder=i18n.t("components.composio.composio_api.tool_name.placeholder"),
        ...
        SortableListInput(
            name="actions",
            display_name=i18n.t("components.composio.composio_api.actions.display_name"),
            placeholder=i18n.t("components.composio.composio_api.actions.placeholder"),
            helper_text=i18n.t("components.composio.composio_api.actions.helper_text"),
    ...
    outputs = [
        Output(
            name="tools",
            display_name=i18n.t("components.composio.composio_api.outputs.tools.display_name"),
            method="build_tool",
        ),
    ]
    ...
    def validate_tool(self, build_config: dict, field_value: Any, tool_name: str | None = None) -> dict:
        logger.debug(
            i18n.t("components.composio.composio_api.logs.validating_tool", tool_name=tool_name or field_value)
        )
        ...
        if selected_tool_index is not None:
            ...
            logger.debug(i18n.t("components.composio.composio_api.logs.tool_validated", index=selected_tool_index))
        ...
        try:
            ...
            if not current_tool:
                warning_msg = i18n.t("components.composio.composio_api.warnings.no_tool_name")
                logger.warning(warning_msg)
                self.log(warning_msg)
                return build_config

            toolkit_slug = current_tool.lower()
            logger.info(i18n.t("components.composio.composio_api.logs.getting_actions", toolkit=toolkit_slug))
            ...
            logger.info(
                i18n.t(
                    "components.composio.composio_api.logs.actions_retrieved",
            ...
            error_msg = i18n.t(
                "components.composio.composio_api.errors.get_actions_failed",
        ...
        logger.debug(
            i18n.t("components.composio.composio_api.logs.actions_config_updated", count=len(authenticated_actions))
        )
        return build_config
    ...
    def update_build_config(self, build_config: dict, field_value: Any, field_name: str | None = None, action: str | None = None) -> dict:
        ...
        if field_name == "api_key" or (self.api_key and not build_config["tool_name"]["options"]):
            if field_name == "api_key" and not field_value:
                logger.info(i18n.t("components.composio.composio_api.logs.clearing_config"))
            ...
            logger.info(i18n.t("components.composio.composio_api.logs.building_tool_list"))
            ...
            logger.info(i18n.t("components.composio.composio_api.logs.tool_list_built", count=len(enabled_tools)))
        ...
        if field_name == "tool_name" and field_value:
            ...
            if not current_tool_name:
                warning_msg = i18n.t("components.composio.composio_api.warnings.no_tool_name_connection")
                logger.warning(warning_msg)
                self.log(warning_msg)
                return build_config
            ...
            try:
                ...
                logger.info(i18n.t("components.composio.composio_api.logs.checking_connection", toolkit=toolkit_slug))
                ...
                if has_active_connections:
                    ...
                    logger.info(
                        i18n.t("components.composio.composio_api.logs.active_connection_found", toolkit=toolkit_slug)
                    )
                    ...
                    logger.info(
                        i18n.t("components.composio.composio_api.logs.no_active_connection", toolkit=toolkit_slug)
                    )
                    ...
                    try:
                        ...
                        if redirect_url and redirect_url.startswith(("http://", "https://")):
                            if selected_tool_index is not None:
                                ...
                                logger.info(
                                    i18n.t(
                                        "components.composio.composio_api.logs.oauth_link_created", toolkit=toolkit_slug
                                    )
                                )
                            ...
                            logger.warning(
                                i18n.t(
                                    "components.composio.composio_api.warnings.invalid_redirect_url",
                                    toolkit=toolkit_slug,
                                )
                            )
                    except (ValueError, ConnectionError, AttributeError) as e:
                        error_msg = i18n.t(
                            "components.composio.composio_api.errors.oauth_creation_failed", error=str(e)
                        )
                        logger.exception(error_msg)
                        self.log(error_msg)
                        if selected_tool_index is not None:
                ...
                error_msg = i18n.t("components.composio.composio_api.errors.connection_check_failed", error=str(e))
                logger.exception(error_msg)
                self.log(error_msg)
    ...
    def build_tool(self) -> Sequence[Tool]:
        ...
        """
        try:
            logger.info(i18n.t("components.composio.composio_api.logs.building_tools"))

            composio = self._build_wrapper()
            action_names = [action["name"] for action in self.actions]

            logger.debug(i18n.t("components.composio.composio_api.logs.actions_selected", count=len(action_names)))
            ...
            if not toolkits:
                warning_msg = i18n.t("components.composio.composio_api.warnings.no_toolkits")
                logger.warning(warning_msg)
            ...
            logger.info(
                i18n.t(
                    "components.composio.composio_api.logs.toolkits_identified",
                    count=len(toolkits),
            ...
            filtered_tools = [tool for tool in all_tools if hasattr(tool, "name") and tool.name in action_names]

            success_msg = i18n.t("components.composio.composio_api.status.tools_built", count=len(filtered_tools))
            ...
            error_msg = i18n.t("components.composio.composio_api.errors.build_tools_failed", error=str(e))
            logger.exception(error_msg)
    ...
    def _build_wrapper(self) -> Composio:
        ...
        try:
            if not self.api_key:
                error_msg = i18n.t("components.composio.composio_api.errors.api_key_required")
                logger.error(error_msg)
                raise ValueError(error_msg)

            logger.debug(i18n.t("components.composio.composio_api.logs.building_wrapper"))
            composio = Composio(api_key=self.api_key, provider=LangchainProvider())
            logger.info(i18n.t("components.composio.composio_api.logs.wrapper_built"))

            return composio

        except ValueError as e:
            error_msg = i18n.t("components.composio.composio_api.errors.wrapper_build_failed", error=str(e))
            logger.exception(error_msg)
            self.log(error_msg)

            invalid_key_msg = i18n.t("components.composio.composio_api.errors.invalid_api_key")
