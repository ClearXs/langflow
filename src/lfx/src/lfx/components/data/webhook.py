...
class WebhookComponent(Component):
    ...
    inputs = [
        MessageTextInput(
            name="webhook_url",
            display_name=i18n.t("components.data.webhook.webhook_url.display_name"),
        ...
        MultilineInput(
            name="payload",
            display_name=i18n.t("components.data.webhook.payload.display_name"),
            info=i18n.t("components.data.webhook.payload.info"),
            placeholder='{"message": "Hello from LangFlow", "timestamp": "{{timestamp}}"}',
        ),
        MultilineInput(
            name="headers",
            display_name=i18n.t("components.data.webhook.headers.display_name"),
        ...
        DropdownInput(
            name="content_type",
            display_name=i18n.t("components.data.webhook.content_type.display_name"),
            info=i18n.t("components.data.webhook.content_type.info"),
            options=["application/json", "application/x-www-form-urlencoded", "text/plain", "application/xml"],
            value="application/json",
            advanced=True,
        ),
        MessageTextInput(
            name="secret_key",
            display_name=i18n.t("components.data.webhook.secret_key.display_name"),
        ...
        DropdownInput(
            ...
            display_name=i18n.t("components.data.webhook.signature_method.display_name"),
        ...
        MessageTextInput(
            ...
            display_name=i18n.t("components.data.webhook.signature_header.display_name"),
        ...
        BoolInput(
            name="verify_ssl",
            display_name=i18n.t("components.data.webhook.verify_ssl.display_name"),
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.data.webhook.follow_redirects.display_name"),
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.data.webhook.include_request_info.display_name"),
        ...
        MessageTextInput(
            name="text_key",
            display_name=i18n.t("components.data.webhook.text_key.display_name"),
    ...
    outputs = [
        Output(
            name="response",
            display_name=i18n.t("components.data.webhook.outputs.response.display_name"),
            method="send_webhook",
        ),
        Output(
            name="request_info",
            display_name=i18n.t("components.data.webhook.outputs.request_info.display_name"),
    ...
    def send_webhook(self) -> list[Data]:
        ...
        try:
            if not self.webhook_url.strip():
                error_message = i18n.t("components.data.webhook.errors.empty_url")
            ...
            success_message = i18n.t(
                "components.data.webhook.success.webhook_sent", status_code=response.status_code, url=self.webhook_url
            )
            ...
            error_message = i18n.t("components.data.webhook.errors.webhook_error", error=str(e))
            self.status = error_message
    ...
    def get_request_info(self) -> Data:
        ...
        try:
            ...
            error_message = i18n.t("components.data.webhook.errors.request_info_error", error=str(e))
            raise ValueError(error_message) from e
    ...
    def _prepare_payload(self) -> str:
        ...
        if self.content_type == "application/json":
            try:
                ...
                error_message = i18n.t("components.data.webhook.errors.invalid_json_payload", error=str(e))
                raise ValueError(error_message) from e
    ...
    def _prepare_headers(self, payload: str) -> dict[str, str]:
        ...
        if self.headers.strip():
            try:
                ...
                warning_message = i18n.t("components.data.webhook.warnings.invalid_headers", error=str(e))
    ...
    def _generate_signature(self, payload: str) -> str:
        ...
        if self.signature_method == "sha256":
            signature = hmac.new(secret_bytes, payload_bytes, hashlib.sha256).hexdigest()
            return f"sha256={signature}"
        if self.signature_method == "md5":
            signature = hmac.new(secret_bytes, payload_bytes, hashlib.md5).hexdigest()
    ...
    def _send_with_retries(self, payload: str, headers: dict[str, str]) -> requests.Response:
        ...
        for attempt in range(max(1, self.retry_count + 1)):
            try:
                ...
                response = requests.request(self.method.upper(), self.webhook_url, **request_kwargs)
                ...
                if attempt < self.retry_count:
                    warning_message = i18n.t(
                        "components.data.webhook.warnings.retry_attempt",
                        attempt=attempt + 1,
        ...
        error_message = i18n.t("components.data.webhook.errors.max_retries_exceeded")
        raise ValueError(error_message)
