...
class RSSComponent(Component):
    ...
    inputs = [
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.data.rss.include_summary.display_name"),
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.data.rss.include_content.display_name"),
        ...
        BoolInput(
            name="parse_dates",
            display_name=i18n.t("components.data.rss.parse_dates.display_name"),
    ...
    outputs = [
        Output(
            name="rss_entries",
            display_name=i18n.t("components.data.rss.outputs.rss_entries.display_name"),
            method="fetch_rss_feed",
        ),
        Output(
            name="feed_info",
            display_name=i18n.t("components.data.rss.outputs.feed_info.display_name"),
        ...
        try:
            ...
            if feed.bozo and hasattr(feed, "bozo_exception"):
                warning_message = i18n.t(
                    "components.data.rss.warnings.feed_parsing_issues", error=str(feed.bozo_exception)
                )
            ...
            success_message = i18n.t(
                "components.data.rss.success.fetched_entries", count=len(result), total=len(feed.entries)
            )
            self.status = success_message
            return result

        except Exception as e:
            error_message = i18n.t("components.data.rss.errors.fetch_error", error=str(e))
            self.status = error_message
    ...
    def get_feed_info(self) -> Data:
        ...
        try:
            ...
            if hasattr(feed.feed, "updated_parsed") and feed.feed.updated_parsed:
                feed_info["updated_datetime"] = datetime(*feed.feed.updated_parsed[:6]).isoformat()

            return Data(data=feed_info, text_key="description")

        except Exception as e:
            error_message = i18n.t("components.data.rss.errors.feed_info_error", error=str(e))
            raise ValueError(error_message) from e
    ...
    def _sort_entries(self, entries: list) -> list:
        ...
        sorted_entries = sorted(entries, key=get_sort_date, reverse=(self.sort_order == "newest_first"))
    ...
    def _parse_entry(self, entry) -> dict:
        ...
        if self.parse_dates:
            if hasattr(entry, "published_parsed") and entry.published_parsed:
                data_dict["published_datetime"] = datetime(*entry.published_parsed[:6]).isoformat()
            if hasattr(entry, "updated_parsed") and entry.updated_parsed:
                data_dict["updated_datetime"] = datetime(*entry.updated_parsed[:6]).isoformat()

        # Add categories/tags if available
        if hasattr(entry, "tags"):
            data_dict["tags"] = [tag.term for tag in entry.tags if hasattr(tag, "term")]

        # Add enclosures (media files) if available
        if hasattr(entry, "enclosures"):
            data_dict["enclosures"] = [
                {"url": enc.href, "type": getattr(enc, "type", ""), "length": getattr(enc, "length", "")}
