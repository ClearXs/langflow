...
class MockDataGeneratorComponent(Component):
    ...
    inputs = [
        IntInput(
            name="number_of_rows",
            display_name=i18n.t("components.data.mock_data.number_of_rows.display_name"),
        ...
        DropdownInput(
            name="data_type",
            display_name=i18n.t("components.data.mock_data.data_type.display_name"),
            info=i18n.t("components.data.mock_data.data_type.info"),
            options=["user_profiles", "products", "transactions", "articles", "custom"],
            value="user_profiles",
            real_time_refresh=True,
        ),
        MessageTextInput(
            name="custom_schema",
            display_name=i18n.t("components.data.mock_data.custom_schema.display_name"),
        ...
        MessageTextInput(
            name="text_key",
            display_name=i18n.t("components.data.mock_data.text_key.display_name"),
    ...
    def generate_mock_data(self) -> list[Data]:
        try:
            ...
            if self.number_of_rows <= 0:
                error_message = i18n.t("components.data.mock_data.errors.invalid_row_count")
            ...
            if self.data_type == "user_profiles":
                ...
                if not self.custom_schema:
                    error_message = i18n.t("components.data.mock_data.errors.missing_custom_schema")
                    self.status = error_message
                    raise ValueError(error_message)
                data_list = self._generate_custom_data()
            else:
                error_message = i18n.t("components.data.mock_data.errors.invalid_data_type", data_type=self.data_type)
                self.status = error_message
                raise ValueError(error_message)

            result = [Data(data=item, text_key=self.text_key) for item in data_list]

            self.status = f"Generated {len(result)} mock data records"
            return result

        except Exception as e:
            error_message = i18n.t("components.data.mock_data.errors.generation_error", error=str(e))
            self.status = error_message
    ...
    def _generate_custom_data(self) -> list[dict]:
        ...
        try:
            ...
            error_message = i18n.t("components.data.mock_data.errors.invalid_schema", error=str(e))
            raise ValueError(error_message) from e
