...
class JSONToDataComponent(Component):
    ...
    inputs = [
        FileInput(
            name="json_file",
            display_name=i18n.t("components.data.json_to_data.json_file.display_name"),
            file_types=["json"],
            info=i18n.t("components.data.json_to_data.json_file.info"),
        ),
        MessageTextInput(
            name="json_path",
            display_name=i18n.t("components.data.json_to_data.json_path.display_name"),
            info=i18n.t("components.data.json_to_data.json_path.info"),
        ),
        MultilineInput(
            name="json_string",
            display_name=i18n.t("components.data.json_to_data.json_string.display_name"),
            info=i18n.t("components.data.json_to_data.json_string.info"),
        ),
        MessageTextInput(
            name="text_key",
            display_name=i18n.t("components.data.json_to_data.text_key.display_name"),
    ...
    outputs = [
        Output(
            name="data_list",
            display_name=i18n.t("components.data.json_to_data.outputs.data_list.display_name"),
    ...
    def load_json_to_data(self) -> list[Data]:
        if sum(bool(field) for field in [self.json_file, self.json_path, self.json_string]) != 1:
            msg = i18n.t("components.data.json_to_data.errors.exactly_one_input")
        ...
        try:
            if self.json_file:
                ...
                if file_path.suffix.lower() != ".json":
                    self.status = i18n.t("components.data.json_to_data.errors.must_be_json")
            ...
            if json_data is not None:
                ...
                if isinstance(json_data, list):
                    result = [Data(data=item, text_key=self.text_key) for item in json_data]
                else:
                    result = [Data(data=json_data, text_key=self.text_key)]

                if not result:
                    self.status = i18n.t("components.data.json_to_data.errors.empty_json")
                    return []
            ...
            error_message = i18n.t("components.data.json_to_data.errors.json_parsing_error", error=str(e))
            self.status = error_message
            raise ValueError(error_message) from e

        except Exception as e:
            error_message = i18n.t("components.data.json_to_data.errors.generic_error", error=str(e))
            self.status = error_message
            raise ValueError(error_message) from e
