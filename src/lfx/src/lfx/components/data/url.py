...
class URLComponent(Component):
    ...
    inputs = [
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.data.url.follow_redirects.display_name"),
        ...
        DropdownInput(
            name="content_type",
            display_name=i18n.t("components.data.url.content_type.display_name"),
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.data.url.extract_metadata.display_name"),
        ...
        BoolInput(
            name="extract_links",
            display_name=i18n.t("components.data.url.extract_links.display_name"),
    ...
    outputs = [
        Output(
            name="data", display_name=i18n.t("components.data.url.outputs.data.display_name"), method="fetch_url_data"
        ),
        Output(
            name="response_info",
            display_name=i18n.t("components.data.url.outputs.response_info.display_name"),
    ...
    def fetch_url_data(self) -> list[Data]:
        ...
        try:
            ...
            if not url_list:
                error_message = i18n.t("components.data.url.errors.no_valid_urls")
                self.status = error_message
                raise ValueError(error_message)
            ...
            for url in url_list:
                try:
                    ...
                    if not parsed_url.scheme or not parsed_url.netloc:
                        warning_message = i18n.t("components.data.url.warnings.invalid_url", url=url)
                    ...
                    results.append(Data(data=data_dict, text_key=self.text_key))
                    successful_requests += 1

                except requests.RequestException as e:
                    warning_message = i18n.t("components.data.url.warnings.request_failed", url=url, error=str(e))
                    self.status = warning_message
                    # Continue with other URLs
                    continue

            if not results:
                error_message = i18n.t("components.data.url.errors.no_successful_requests")
                self.status = error_message
                return []

            success_message = i18n.t(
                "components.data.url.success.fetched_data", successful=successful_requests, total=len(url_list)
            )
            self.status = success_message
            return results

        except Exception as e:
            error_message = i18n.t("components.data.url.errors.fetch_error", error=str(e))
            self.status = error_message
        ...
        try:
            ...
            for url in url_list:
                try:
                    ...
                    error_dict = {"url": url, "error": str(e), "status": "failed"}
                    results.append(Data(data=error_dict, text_key="url"))

            return results

        except Exception as e:
            error_message = i18n.t("components.data.url.errors.response_info_error", error=str(e))
            raise ValueError(error_message) from e
    ...
    def _parse_headers(self) -> dict[str, str]:
        ...
        try:
            ...
            warning_message = i18n.t("components.data.url.warnings.invalid_headers", error=str(e))
            self.status = warning_message
            return {}

    def _parse_body(self) -> str | None:
        """Parse body content."""
    ...
    def _make_request(self, url: str, headers: dict[str, str], body: str | None) -> requests.Response:
        ...
        if len(response.content) > self.max_content_length:
            warning_message = i18n.t(
                "components.data.url.warnings.content_too_large",
