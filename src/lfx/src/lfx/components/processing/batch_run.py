...
class BatchRunComponent(Component):
    ...
    inputs = [
        HandleInput(
            name="model",
            display_name=i18n.t("components.processing.batch_run.model.display_name"),
        ...
        MultilineInput(
            name="system_message",
            display_name=i18n.t("components.processing.batch_run.system_message.display_name"),
            info=i18n.t("components.processing.batch_run.system_message.info"),
            required=False,
        ),
        DataFrameInput(
            name="df",
            display_name=i18n.t("components.processing.batch_run.df.display_name"),
            info=i18n.t("components.processing.batch_run.df.info"),
            required=True,
        ),
        MessageTextInput(
            name="column_name",
            display_name=i18n.t("components.processing.batch_run.column_name.display_name"),
        ...
        MessageTextInput(
            ...
            display_name=i18n.t("components.processing.batch_run.output_column_name.display_name"),
            info=i18n.t("components.processing.batch_run.output_column_name.info"),
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.processing.batch_run.enable_metadata.display_name"),
            info=i18n.t("components.processing.batch_run.enable_metadata.info"),
            value=False,
            required=False,
            advanced=True,
        ),
    ...
    outputs = [
        Output(
            display_name=i18n.t("components.processing.batch_run.outputs.batch_results.display_name"),
            name="batch_results",
            method="run_batch",
            info=i18n.t("components.processing.batch_run.outputs.batch_results.info"),
        ),
    ]
    ...
    def _format_row_as_toml(self, row: dict[str, Any]) -> str:
        ...
        try:
            ...
            error_msg = i18n.t("components.processing.batch_run.errors.toml_formatting_failed", error=str(e))
    ...
    async def run_batch(self) -> DataFrame:
        ...
        try:
            ...
            if not isinstance(df, DataFrame):
                error_msg = i18n.t(
                    "components.processing.batch_run.errors.invalid_dataframe_type", actual_type=type(df).__name__
                )
            ...
            if col_name and col_name not in df.columns:
                ...
                error_msg = i18n.t(
                    "components.processing.batch_run.errors.column_not_found", column=col_name, available=available_cols
                )
                self.status = error_msg
            ...
            if col_name:
                ...
                info_msg = i18n.t(
                    "components.processing.batch_run.info.processing_column", column=col_name, count=len(user_texts)
                )
            else:
                user_texts = [
                    self._format_row_as_toml(cast("dict[str, Any]", row)) for row in df.to_dict(orient="records")
                ]
                info_msg = i18n.t("components.processing.batch_run.info.processing_all_columns", count=len(user_texts))

            total_rows = len(user_texts)

            processing_msg = i18n.t("components.processing.batch_run.info.starting_batch", total=total_rows)
            ...
            for idx, (original_row, response) in enumerate(
                ...
                if (idx + 1) % progress_interval == 0:
                    progress_msg = i18n.t(
                        "components.processing.batch_run.info.processing_progress", current=idx + 1, total=total_rows
                    )
                    await logger.ainfo(progress_msg)

            success_msg = i18n.t("components.processing.batch_run.success.batch_completed", total=total_rows)
            ...
            error_msg = i18n.t("components.processing.batch_run.errors.data_processing_error", error=str(e))
            self.status = error_msg
            ...
            error_msg = i18n.t("components.processing.batch_run.errors.unexpected_error", error=str(e))
            self.status = error_msg
            await logger.aerror(error_msg)

            # Create error DataFrame with empty columns
            error_row = self._create_base_row({}, model_response="", batch_index=-1)
            self._add_metadata(error_row, success=False, error=str(e))
            return DataFrame([error_row])
