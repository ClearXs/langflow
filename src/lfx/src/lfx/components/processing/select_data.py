...
class SelectDataComponent(Component):
    ...
    display_name: str = i18n.t("components.processing.select_data.display_name")
    ...
    inputs = [
        DataInput(
            name="data_list",
            display_name=i18n.t("components.processing.select_data.data_list.display_name"),
        ...
        IntInput(
            name="data_index",
            display_name=i18n.t("components.processing.select_data.data_index.display_name"),
    ...
    outputs = [
        Output(
            display_name=i18n.t("components.processing.select_data.outputs.selected_data.display_name"),
            name="selected_data",
            method="select_data",
        ),
    ]
    ...
    async def select_data(self) -> Data:
        ...
        try:
            ...
            if not hasattr(self, "data_list") or not self.data_list:
                error_msg = i18n.t("components.processing.select_data.errors.empty_data_list")
                self.status = error_msg
                raise ValueError(error_msg)

            if not isinstance(self.data_list, list):
                error_msg = i18n.t("components.processing.select_data.errors.invalid_data_list_type")
                self.status = error_msg
                raise ValueError(error_msg)

            # Validate index
            if not hasattr(self, "data_index"):
                error_msg = i18n.t("components.processing.select_data.errors.missing_data_index")
                self.status = error_msg
                raise ValueError(error_msg)
            ...
            try:
                ...
                error_msg = i18n.t(
                    "components.processing.select_data.errors.invalid_index_type", index=self.data_index, error=str(e)
                )
                self.status = error_msg
            ...
            if selected_index < 0:
                error_msg = i18n.t("components.processing.select_data.errors.negative_index", index=selected_index)
                self.status = error_msg
                raise ValueError(error_msg)

            if selected_index >= data_list_length:
                error_msg = i18n.t(
                    "components.processing.select_data.errors.index_out_of_range",
                    index=selected_index,
                    max_index=data_list_length - 1,
            ...
            if not isinstance(selected_data, Data):
                error_msg = i18n.t(
                    "components.processing.select_data.errors.invalid_selected_data_type",
                    index=selected_index,
                    actual_type=type(selected_data).__name__,
                )
                self.status = error_msg
                raise ValueError(error_msg)
            ...
            success_msg = i18n.t(
                "components.processing.select_data.success.data_selected", index=selected_index, total=data_list_length
            )
            ...
            self.log(
                i18n.t(
                    "components.processing.select_data.logs.data_selected",
                    index=selected_index,
                    data_preview=str(selected_data)[:100],
                )
            )
            ...
            error_msg = i18n.t("components.processing.select_data.errors.selection_failed", error=str(e))
            self.status = error_msg
    ...
    def update_build_config(self, build_config, field_value, field_name=None):
        ...
        try:
            ...
            error_msg = i18n.t("components.processing.select_data.errors.build_config_update_failed", error=str(e))
