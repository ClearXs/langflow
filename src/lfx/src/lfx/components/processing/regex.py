...
class RegexExtractorComponent(Component):
    ...
    inputs = [
        MessageTextInput(
            name="input_text",
            display_name=i18n.t("components.processing.regex.input_text.display_name"),
            info=i18n.t("components.processing.regex.input_text.info"),
            required=True,
        ),
        MessageTextInput(
            name="pattern",
            display_name=i18n.t("components.processing.regex.pattern.display_name"),
    ...
    outputs = [
        Output(
            display_name=i18n.t("components.processing.regex.outputs.data.display_name"),
            name="data",
            method="extract_matches",
        ),
        Output(
            display_name=i18n.t("components.processing.regex.outputs.message.display_name"),
            name="text",
    ...
    def extract_matches(self) -> list[Data]:
        ...
        try:
            ...
            if not self.pattern or not self.pattern.strip():
                warning_msg = i18n.t("components.processing.regex.warnings.empty_pattern")
                self.status = warning_msg
                return []

            if not self.input_text or not self.input_text.strip():
                warning_msg = i18n.t("components.processing.regex.warnings.empty_input_text")
                self.status = warning_msg
                return []
            ...
            try:
                ...
                error_msg = i18n.t("components.processing.regex.errors.invalid_pattern", error=str(e))
            ...
            filtered_matches = [match for match in matches if match and str(match).strip()]

            if not filtered_matches:
                warning_msg = i18n.t("components.processing.regex.warnings.no_matches")
                self.status = warning_msg
                return []
            ...
            for i, match in enumerate(filtered_matches):
                try:
                    ...
                    if isinstance(match, tuple):
                        ...
                        match_data = {f"group_{j}": group for j, group in enumerate(match)}
                        match_data["full_match"] = "".join(str(g) for g in match if g)
                    else:
                    ...
                    error_msg = i18n.t(
                        "components.processing.regex.errors.match_processing_failed", index=i, error=str(e)
                    )
                    self.log(error_msg, "warning")
                    # Continue with other matches

            success_msg = i18n.t(
                "components.processing.regex.success.matches_extracted", pattern=self.pattern, count=len(result)
            )
            ...
            error_msg = i18n.t("components.processing.regex.errors.extraction_failed", error=str(e))
            self.status = error_msg
    ...
    def get_matches_text(self) -> Message:
        ...
        try:
            ...
            if not matches:
                message_text = i18n.t("components.processing.regex.messages.no_matches_found")
            ...
            for i, match in enumerate(matches):
                try:
                    ...
                    error_msg = i18n.t(
                        "components.processing.regex.errors.match_formatting_failed", index=i, error=str(e)
                    )
                    self.log(error_msg, "warning")
                    # Continue with other matches

            if not formatted_matches:
                message_text = i18n.t("components.processing.regex.messages.no_valid_matches")
                message = Message(text=message_text)
                self.status = message_text
                return message
            ...
            success_msg = i18n.t(
                "components.processing.regex.success.text_formatted",
            ...
            error_msg = i18n.t("components.processing.regex.errors.text_formatting_failed", error=str(e))
            self.status = error_msg
            self.log(error_msg, "error")
