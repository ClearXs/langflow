...
class MergeDataComponent(Component):
    ...
    inputs = [
        DataInput(
            name="data_inputs",
            display_name=i18n.t("components.processing.merge_data.data_inputs.display_name"),
        ...
        DropdownInput(
            name="operation",
            display_name=i18n.t("components.processing.merge_data.operation.display_name"),
            options=[
            ...
            value=i18n.t("components.processing.merge_data.operations.concatenate"),
    ...
    outputs = [
        Output(
            display_name=i18n.t("components.processing.merge_data.outputs.dataframe.display_name"),
            name="combined_data",
            method="combine_data",
        )
    ]
    ...
    def combine_data(self) -> DataFrame:
        try:
            ...
            if not self.data_inputs:
                warning_msg = i18n.t("components.processing.merge_data.warnings.no_data_inputs")
                self.status = warning_msg
                return DataFrame()

            if len(self.data_inputs) < self.MIN_INPUTS_REQUIRED:
                warning_msg = i18n.t(
                    "components.processing.merge_data.warnings.insufficient_inputs",
                    required=self.MIN_INPUTS_REQUIRED,
                    provided=len(self.data_inputs),
                )
                self.status = warning_msg
                return DataFrame()
            ...
            operation_enum = operation_map.get(self.operation, DataOperation.CONCATENATE)

            combined_dataframe = self._process_operation(operation_enum)

            success_msg = i18n.t(
                "components.processing.merge_data.success.data_combined",
                operation=self.operation,
                inputs=len(self.data_inputs),
            ...
            error_msg = i18n.t(
                "components.processing.merge_data.errors.combination_failed", operation=self.operation, error=str(e)
            )
    ...
    def _process_operation(self, operation: DataOperation) -> DataFrame:
        ...
        try:
            ...
            error_msg = i18n.t(
                "components.processing.merge_data.errors.unsupported_operation", operation=operation.value
            )
            raise ValueError(error_msg)

        except Exception as e:
            error_msg = i18n.t(
                "components.processing.merge_data.errors.operation_processing_failed",
    ...
    def _concatenate_data(self) -> DataFrame:
        ...
        try:
            ...
            error_msg = i18n.t("components.processing.merge_data.errors.concatenate_failed", error=str(e))
            raise ValueError(error_msg) from e
    ...
    def _append_data(self) -> DataFrame:
        ...
        try:
            ...
            error_msg = i18n.t("components.processing.merge_data.errors.append_failed", error=str(e))
            raise ValueError(error_msg) from e
    ...
    def _merge_data(self) -> DataFrame:
        ...
        try:
            ...
            error_msg = i18n.t("components.processing.merge_data.errors.merge_failed", error=str(e))
            raise ValueError(error_msg) from e
    ...
    def _join_data(self) -> DataFrame:
        ...
        try:
            ...
            error_msg = i18n.t("components.processing.merge_data.errors.join_failed", error=str(e))
            raise ValueError(error_msg) from e
