...
class DoclingInlineComponent(BaseFileComponent):
    ...
    inputs = [
        *BaseFileComponent.get_base_inputs(),
        DropdownInput(
            name="pipeline",
            display_name=i18n.t("components.docling.docling_inline.pipeline.display_name"),
        ...
        DropdownInput(
            name="ocr_engine",
            display_name=i18n.t("components.docling.docling_inline.ocr_engine.display_name"),
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.docling.docling_inline.do_picture_classification.display_name"),
            info=i18n.t("components.docling.docling_inline.do_picture_classification.info"),
            value=False,
        ),
        HandleInput(
            name="pic_desc_llm",
            display_name=i18n.t("components.docling.docling_inline.pic_desc_llm.display_name"),
        ...
        StrInput(
            ...
            display_name=i18n.t("components.docling.docling_inline.pic_desc_prompt.display_name"),
            value="Describe the image in three sentences. Be concise and accurate.",
            info=i18n.t("components.docling.docling_inline.pic_desc_prompt.info"),
            advanced=True,
        ),
    ...
    def _wait_for_result_with_process_monitoring(self, queue: Queue, proc, timeout: int = 300):
        ...
        logger.debug(i18n.t("components.docling.docling_inline.logs.monitoring_process", timeout=timeout))
        ...
        while time.time() - start_time < timeout:
            ...
            if not proc.is_alive():
                ...
                try:
                    ...
                    error_msg = i18n.t(
                        "components.docling.docling_inline.errors.process_crashed", exit_code=proc.exitcode
                    )
                    logger.error(error_msg)
                    raise RuntimeError(error_msg) from None
                else:
                    logger.info(i18n.t("components.docling.docling_inline.logs.process_completed"))
                    self.log(i18n.t("components.docling.docling_inline.logs.process_completed"))
                    return result
            ...
            try:
                ...
                logger.info(i18n.t("components.docling.docling_inline.logs.result_received"))
                self.log(i18n.t("components.docling.docling_inline.logs.result_received"))
                return result

        # Overall timeout reached
        error_msg = i18n.t("components.docling.docling_inline.errors.process_timeout", timeout=timeout)
        logger.error(error_msg)
    ...
    def _terminate_process_gracefully(self, proc, timeout_terminate: int = 10, timeout_kill: int = 5):
        ...
        logger.info(i18n.t("components.docling.docling_inline.logs.terminating_sigterm"))
        self.log(i18n.t("components.docling.docling_inline.logs.terminating_sigterm"))
        proc.terminate()  # Send SIGTERM
        proc.join(timeout=timeout_terminate)

        if proc.is_alive():
            logger.warning(i18n.t("components.docling.docling_inline.logs.terminating_sigkill"))
            self.log(i18n.t("components.docling.docling_inline.logs.terminating_sigkill"))
            proc.kill()  # Send SIGKILL
            proc.join(timeout=timeout_kill)

            if proc.is_alive():
                warning_msg = i18n.t("components.docling.docling_inline.logs.process_still_alive")
    ...
    def process_files(self, file_list: list[BaseFileComponent.BaseFile]) -> list[BaseFileComponent.BaseFile]:
        ...
        try:
            ...
            logger.debug(i18n.t("components.docling.docling_inline.logs.docling_import_successful"))
        except ImportError as e:
            error_msg = i18n.t("components.docling.docling_inline.errors.docling_import_failed")
            logger.error(error_msg)
        ...
        if not file_paths:
            logger.info(i18n.t("components.docling.docling_inline.logs.no_files"))
            self.log(i18n.t("components.docling.docling_inline.logs.no_files"))
            return file_list

        logger.info(i18n.t("components.docling.docling_inline.logs.processing_files", count=len(file_paths)))
        ...
        if self.pic_desc_llm is not None:
            ...
            logger.debug(i18n.t("components.docling.docling_inline.logs.pic_desc_enabled"))
        ...
        logger.debug(
            i18n.t(
                "components.docling.docling_inline.logs.creating_worker",
        ...
        result = None
        proc.start()
        logger.info(i18n.t("components.docling.docling_inline.logs.worker_started"))

        try:
            result = self._wait_for_result_with_process_monitoring(queue, proc, timeout=300)
        except KeyboardInterrupt:
            logger.info(i18n.t("components.docling.docling_inline.logs.cancelled_by_user"))
            self.log(i18n.t("components.docling.docling_inline.logs.cancelled_by_user"))
            result = []
        except Exception as e:
            error_msg = i18n.t("components.docling.docling_inline.errors.processing_error", error=str(e))
            ...
            try:
                ...
                try:
                    ...
                    warning_msg = i18n.t("components.docling.docling_inline.logs.cleanup_warning", error=str(e))
                    logger.warning(warning_msg)
                    self.log(warning_msg)
        ...
        if isinstance(result, dict) and "error" in result:
            ...
            if result.get("error_type") == "dependency_error":
                ...
                user_message = i18n.t(
                    "components.docling.docling_inline.errors.missing_ocr_dependency",
            ...
            if "Worker interrupted by SIGINT" in error_msg or "shutdown" in result:
                ...
                logger.error(i18n.t("components.docling.docling_inline.errors.runtime_error", error=error_msg))
                raise RuntimeError(error_msg)

        logger.info(i18n.t("components.docling.docling_inline.logs.processing_completed", count=len(result)))
