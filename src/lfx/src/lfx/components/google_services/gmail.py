...
class GmailLoaderComponent(Component):
    ...
    inputs = [
        SecretStrInput(
            name="json_string",
            display_name=i18n.t("components.google.gmail.json_string.display_name"),
        ...
        MessageTextInput(
            name="label_ids",
            display_name=i18n.t("components.google.gmail.label_ids.display_name"),
        ...
        MessageTextInput(
            name="max_results",
            display_name=i18n.t("components.google.gmail.max_results.display_name"),
    ...
    outputs = [
        Output(
            display_name=i18n.t("components.google.gmail.outputs.data.display_name"), name="data", method="load_emails"
        ),
    ]
    ...
    def load_emails(self) -> Data:
        ...
        class CustomGMailLoader(GMailLoader):
            def __init__(
                ...
                logger.debug(
                    i18n.t(
                        "components.google.gmail.logs.loader_initialized",
                        max_results=n,
            ...
            def clean_message_content(self, message):
                ...
                logger.debug(i18n.t("components.google.gmail.logs.cleaning_message", length=len(message)))
                ...
                logger.debug(
                    i18n.t(
                        "components.google.gmail.logs.message_cleaned",
            ...
            def _extract_email_content(self, msg: Any) -> HumanMessage:
                ...
                logger.debug(i18n.t("components.google.gmail.logs.extracting_content"))
                ...
                if from_email is None:
                    error_msg = i18n.t("components.google.gmail.errors.from_email_not_found")
                    logger.error(error_msg)
                    raise ValueError(error_msg)

                logger.debug(i18n.t("components.google.gmail.logs.from_email_found", email=from_email))
                ...
                for part in parts:
                    if part["mimeType"] == "text/plain":
                        ...
                        logger.debug(i18n.t("components.google.gmail.logs.content_extracted"))
                ...
                error_msg = i18n.t("components.google.gmail.errors.no_plain_text")
                logger.error(error_msg)
                raise ValueError(error_msg)

            def _get_message_data(self, service: Any, message: Any) -> ChatSession:
                """Get message data including thread context if available."""
                logger.debug(i18n.t("components.google.gmail.logs.getting_message_data", message_id=message["id"]))
                ...
                if in_reply_to:
                    logger.debug(i18n.t("components.google.gmail.logs.processing_thread", thread_id=thread_id))
                    ...
                    if response_email is None:
                        error_msg = i18n.t("components.google.gmail.errors.response_email_not_found")
                        logger.warning(error_msg)
                        raise ValueError(error_msg)

                    starter_content = self._extract_email_content(response_email)
                    logger.debug(i18n.t("components.google.gmail.logs.thread_context_added"))
                    return ChatSession(messages=[starter_content, message_content])

                logger.debug(i18n.t("components.google.gmail.logs.single_message"))
                return ChatSession(messages=[message_content])

            def lazy_load(self) -> Iterator[ChatSession]:
                """Lazy load emails from Gmail."""
                logger.info(i18n.t("components.google.gmail.logs.starting_lazy_load"))
                ...
                if not messages:
                    warning_msg = i18n.t("components.google.gmail.logs.no_messages_found")
                    logger.warning(warning_msg)

                logger.info(i18n.t("components.google.gmail.logs.messages_found", count=len(messages)))

                for idx, message in enumerate(messages, 1):
                    try:
                        logger.debug(
                            i18n.t(
                                "components.google.gmail.logs.processing_message",
                                index=idx,
                                total=len(messages),
                        ...
                        if self.raise_error:
                            ...
                            logger.exception(
                                i18n.t(
                                    "components.google.gmail.logs.message_processing_error",
                                    message_id=message["id"],
                                    error=str(e),
                                )
                            )
        ...
        logger.debug(
            i18n.t(
                "components.google.gmail.logs.parameters_parsed", labels=",".join(label_ids), max_results=max_results
            )
        )
        ...
        try:
            logger.debug(i18n.t("components.google.gmail.logs.creating_credentials"))
            creds = Credentials.from_authorized_user_info(token_info)
            logger.debug(i18n.t("components.google.gmail.logs.credentials_created"))
        except Exception as e:
            error_msg = i18n.t("components.google.gmail.errors.credentials_creation_failed", error=str(e))
        ...
        try:
            logger.info(i18n.t("components.google.gmail.logs.loading_documents"))
            docs = loader.load()
            logger.info(i18n.t("components.google.gmail.logs.documents_loaded", count=len(docs)))
        except RefreshError as e:
            error_msg = i18n.t("components.google.gmail.errors.auth_refresh_failed")
            logger.error(error_msg)
            raise ValueError(error_msg) from e
        except Exception as e:
            error_msg = i18n.t("components.google.gmail.errors.loading_failed", error=str(e))
            logger.exception(error_msg)
            raise ValueError(error_msg) from e
