    ...
    inputs = [
        ...
        TableInput(
            ...
            table_schema=[
                ...
                {"name": "masking_type", "display_name": "Masking Type", "type": "str"},
                {"name": "mask_char", "display_name": "Mask Character", "type": "str"},
            ],
            value=[],
            required=True,
        ),
                ...
                if field in df.columns:
                    if masking_type == "phone":
                        df[field] = df[field].apply(
                            lambda x: self._mask_phone(str(x), mask_char) if pd.notnull(x) else x
                        )
                    elif masking_type == "email":
                        df[field] = df[field].apply(
                            lambda x: self._mask_email(str(x), mask_char) if pd.notnull(x) else x
                        )
