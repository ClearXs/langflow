"""MySQL Output Component - 写入数据到 MySQL 数据库表"""
...
class MySQLOutputComponent(BaseSQLOutputComponent):
    ...
    display_name = i18n.t("components.input_output.databases.mysql_output.display_name")
    description = i18n.t("components.input_output.databases.mysql_output.description")
    icon = "MySQL"
    name = "MySQLOutput"
    ...
    inputs = [
        DataInput(
            name="data_input",
            display_name=i18n.t("components.input_output.databases.mysql_output.data_input.display_name"),
            info=i18n.t("components.input_output.databases.mysql_output.data_input.info"),
            required=True,
            is_list=True,
        ),
        DropdownInput(
            name="datasource_selector",
            display_name=i18n.t("components.input_output.databases.mysql_output.datasource_selector.display_name"),
            info=i18n.t("components.input_output.databases.mysql_output.datasource_selector.info"),
            required=True,
            refresh_button=True,
            options=[],
        ...
        DropdownInput(
            name="table_name",
            display_name=i18n.t("components.input_output.databases.mysql_output.table_name.display_name"),
            info=i18n.t("components.input_output.databases.mysql_output.table_name.info"),
            required=True,
            refresh_button=True,
            options=[],
        ),
        TableInput(
            name="field_mappings",
            display_name=i18n.t("components.input_output.databases.mysql_output.field_mappings.display_name"),
            info=i18n.t("components.input_output.databases.mysql_output.field_mappings.info"),
            table_schema=[
                {
                    "name": "source_field",
                    "display_name": i18n.t(
                        "components.input_output.databases.mysql_output.field_mappings.source_field"
                    ),
                    "type": "str",
                    "disable_edit": True,
                },
                {
                    "name": "target_field",
                    "display_name": i18n.t(
                        "components.input_output.databases.mysql_output.field_mappings.target_field"
                    ),
                    "type": "str",
                },
                {
                    "name": "data_type",
                    "display_name": i18n.t(
                        "components.input_output.databases.mysql_output.field_mappings.data_type"
                    ),
                    "type": "str",
                    "disable_edit": True,
                    "formatter": "code",
                ...
                },
                {
                    "name": "update_option",
                    "display_name": i18n.t(
                        "components.input_output.databases.mysql_output.field_mappings.update_option"
                    ),
                    "type": "str",
                ...
                },
                {
                    "name": "is_key_field",
                    "display_name": i18n.t(
                        "components.input_output.databases.mysql_output.field_mappings.is_key_field"
                    ),
                    "type": "bool",
                    "formatter": "boolean",
                    "description": i18n.t(
                        "components.input_output.databases.mysql_output.field_mappings.is_key_field_desc"
                    ),
                },
                {
                    "name": "null_value",
                    "display_name": i18n.t(
                        "components.input_output.databases.mysql_output.field_mappings.null_value"
                    ),
                    "type": "str",
                    "description": i18n.t(
                        "components.input_output.databases.mysql_output.field_mappings.null_value_desc"
                    ),
                },
            ],
            value=[],
                ...
                "action_buttons": [
                    {
                        "name": "analyze_schema",
                        "label": i18n.t(
                            "components.input_output.databases.mysql_output.field_mappings.analyze_schema_button"
                        ),
                        "icon": "Search",
                        "position": "top",
                    }
        ...
        TableInput(
            name="preview_table",
            display_name=i18n.t("components.input_output.databases.mysql_output.preview_table.display_name"),
            info=i18n.t("components.input_output.databases.mysql_output.preview_table.info"),
            table_schema=[],
            value=[],
            table_options={
                "block_add": True,
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.input_output.databases.mysql_output.clear_table_first.display_name"),
            info=i18n.t("components.input_output.databases.mysql_output.clear_table_first.info"),
            value=False,
            advanced=True,
        ),
        IntInput(
            name="batch_size",
            display_name=i18n.t("components.input_output.databases.mysql_output.batch_size.display_name"),
            info=i18n.t("components.input_output.databases.mysql_output.batch_size.info"),
            value=1000,
            advanced=True,
        ),
    ]
    ...
    outputs = [
        Output(
            name="data",
            display_name=i18n.t("components.input_output.databases.mysql_output.outputs.data.display_name"),
    ...
    def update_build_config(
        ...
        # 3. 处理 analyze_schema 动作按钮
            ...
            logger.info("[MySQLOutput] Schema analysis triggered")
            ...
            try:
                ...
                if not graph_data and hasattr(self, "graph") and self.graph is not None:
                    if hasattr(self.graph, "data"):
                        ...
                        logger.warning("[MySQLOutput] PlaceholderGraph detected - no graph data available")

                if not graph_data:
                    logger.warning("[MySQLOutput] No graph data available for schema analysis")
                    self.status = i18n.t("components.input_output.databases.mysql_output.errors.no_graph_data")
                    return build_config

                # 使用静态字段提取（不执行上游组件）
                self.status = i18n.t("components.input_output.databases.mysql_output.status.analyzing_schema")
                logger.info("[MySQLOutput] Using static field extraction from upstream component")
                ...
                try:
                    ...
                    if not upstream_node_id:
                        logger.warning("[MySQLOutput] No upstream node found for field analysis")
                    ...
                    if not field_names:
                        logger.warning("[MySQLOutput] No fields extracted from upstream configuration")
                        self.status = i18n.t("components.input_output.databases.mysql_output.status.no_fields_found")
                        return build_config

                    logger.info(f"[MySQLOutput] Extracted {len(field_names)} fields from upstream: {field_names}")
                    ...
                    logger.info(
                        f"[MySQLOutput] Static field analysis completed, generated {len(field_info)} field mappings"
                    )
                    self.status = i18n.t(
                        "components.input_output.databases.mysql_output.status.analysis_success", count=len(field_info)
                    )

                except Exception as e:
                    logger.error(f"[MySQLOutput] Static field extraction failed: {e}")
                    import traceback

                    logger.error(f"[MySQLOutput] Traceback: {traceback.format_exc()}")
                    self.status = i18n.t(
                        "components.input_output.databases.mysql_output.errors.analysis_failed", error=str(e)
                    )
                ...
                logger = logging.getLogger(__name__)
                error_msg = str(e)
                logger.warning(f"[MySQLOutput] Schema analysis warning: {error_msg}")
                self.status = i18n.t(
                ...
                logger = logging.getLogger(__name__)
                error_msg = str(e)
                logger.error(f"[MySQLOutput] Unexpected error during schema analysis: {error_msg}")
                import traceback
    ...
    async def write_to_table(self) -> list[Data]:
        ...
        try:
            ...
            if not datasource_id:
                raise ValueError(i18n.t("components.input_output.databases.mysql_output.errors.no_datasource"))
            ...
            if not self.datasource_obj:
                raise ValueError(
                    i18n.t(
                        "components.input_output.databases.mysql_output.errors.datasource_not_found",
                        id=datasource_id,
                    )
                )

            # 更新状态
            self.status = i18n.t("components.input_output.databases.mysql_output.status.writing")
            ...
            error_msg = i18n.t("components.input_output.databases.mysql_output.errors.write_failed", error=str(e))
