...
class ETLKafkaInputComponent(Component):
    ...
    def update_build_config(
        ...
        logger.info(f"[KafkaInput] update_build_config called - field_name: {field_name}, field_value: {field_value}")
        ...
        if field_name is None or field_name == "datasource_selector":
            logger.debug(f"[KafkaInput] Loading Kafka datasources (field_name={field_name})")
            try:
                ...
                logger.debug(f"[KafkaInput] Saved build_config with {len(options)} datasources to instance")
        ...
        try:
            ...
            if hasattr(self, "_build_config") and self._build_config:
                ...
                for option in options_metadata:
                    if option.get("id") == datasource_id or option.get("value") == datasource_id:
                        logger.debug(f"[KafkaInput] Found datasource info from _build_config for {datasource_id}")
                        return option
            ...
            if hasattr(self, "_vertex") and self._vertex:
                try:
                    ...
                    for option in options_metadata:
                        if option.get("id") == datasource_id or option.get("value") == datasource_id:
                            logger.debug(f"[KafkaInput] Found datasource info from vertex template for {datasource_id}")
                            return option
                except Exception as e:
                    logger.warning(f"[KafkaInput] Error accessing vertex template: {e}")

            # Log detailed info when not found
            logger.warning(
                f"[KafkaInput] Datasource {datasource_id} not found. "
            ...
            logger.error(f"[KafkaInput] Error finding datasource info: {e}", exc_info=True)
            return None
    ...
    def _get_kafka_config_from_datasource(self) -> dict:
        ...
        try:
            ...
            logger.debug(
                f"[KafkaInput] Datasource info: source={datasource_info.get('source')}, "
            ...
            logger.info(f"[KafkaInput] Fetching builtin datasource via API for ID: {datasource_id}")
            api_url = os.getenv("LANGFLOW_API_URL", "http://localhost:7860")
            logger.debug(f"[KafkaInput] API URL: {api_url}/api/v1/datasources/{datasource_id}")
    ...
    async def _consume_messages_streaming_async(self) -> AsyncGenerator[Data, None]:
        ...
        try:
            ...
            # 2. 复用或创建 Consumer
            if self._current_consumer is None:
                # Run synchronous _create_consumer in a thread to avoid blocking event loop
                logger.debug("[KafkaInput] Creating new consumer in thread pool (async context)")
                consumer = await asyncio.to_thread(self._create_consumer)
                self._current_consumer = consumer
            else:
                consumer = self._current_consumer
                logger.info("Reusing existing consumer")
