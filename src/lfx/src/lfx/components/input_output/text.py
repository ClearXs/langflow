...
from lfx.inputs.inputs import BoolInput, DropdownInput, IntInput, MessageTextInput, MultilineInput
...
class TextComponent(Component):
    ...
    inputs = [
        MultilineInput(
            name="text",
            display_name=i18n.t("components.input_output.text.text.display_name"),
        ...
        DropdownInput(
            name="text_type",
            display_name=i18n.t("components.input_output.text.text_type.display_name"),
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.input_output.text.strip_whitespace.display_name"),
        ...
        BoolInput(
            ...
            display_name=i18n.t("components.input_output.text.validate_format.display_name"),
        ...
        MessageTextInput(
            ...
            display_name=i18n.t("components.input_output.text.template_variables.display_name"),
            info=i18n.t("components.input_output.text.template_variables.info"),
        ...
        MessageTextInput(
            name="regex_pattern",
            display_name=i18n.t("components.input_output.text.regex_pattern.display_name"),
        ...
        BoolInput(
            name="case_sensitive",
            display_name=i18n.t("components.input_output.text.case_sensitive.display_name"),
        ...
        DropdownInput(
            name="encoding",
            display_name=i18n.t("components.input_output.text.encoding.display_name"),
    ...
    outputs = [
        Output(
            display_name=i18n.t("components.input_output.text.outputs.text.display_name"),
            name="text",
            method="get_text",
        ),
        Output(
            display_name=i18n.t("components.input_output.text.outputs.data.display_name"),
            name="data",
            method="get_data",
        ),
        Output(
            display_name=i18n.t("components.input_output.text.outputs.processed_text.display_name"),
            name="processed_text",
    ...
    def _validate_inputs(self) -> None:
        ...
        if self.min_length > 0 and len(self.text) < self.min_length:
            error_message = i18n.t(
                "components.input_output.text.errors.text_too_short", length=len(self.text), min_length=self.min_length
            )
        ...
        if self.max_length > 0 and len(self.text) > self.max_length:
            error_message = i18n.t(
                "components.input_output.text.errors.text_too_long", length=len(self.text), max_length=self.max_length
            )
            self.status = error_message
            raise ValueError(error_message)
        ...
        if self.validate_format and self.regex_pattern:
            try:
                ...
                if not re.search(self.regex_pattern, self.text, flags):
                    error_message = i18n.t(
                        "components.input_output.text.errors.regex_validation_failed", pattern=self.regex_pattern
                    )
                    self.status = error_message
                    raise ValueError(error_message)
            except re.error as e:
                error_message = i18n.t(
                    "components.input_output.text.errors.invalid_regex_pattern",
    ...
    def _validate_format(self, text: str) -> bool:
        ...
        try:
            if self.text_type == "json":
                ...
                markdown_patterns = [r"#", r"\*", r"_", r"`", r"\[.*\]\(.*\)", r"!\[.*\]\(.*\)"]
    ...
    def _process_template(self, text: str) -> str:
        ...
        try:
            ...
            warning_message = i18n.t("components.input_output.text.warnings.invalid_template_variables", error=str(e))
            self.status = warning_message
            return text
        except Exception as e:
            warning_message = i18n.t("components.input_output.text.warnings.template_processing_error", error=str(e))
            self.status = warning_message
            return text
    ...
    def _clean_text(self, text: str) -> str:
        ...
        # Process encoding
        try:
            processed_text = processed_text.encode(self.encoding).decode(self.encoding)
        except UnicodeError as e:
            warning_message = i18n.t(
                "components.input_output.text.warnings.encoding_error", encoding=self.encoding, error=str(e)
            )
            self.status = warning_message
    ...
    def get_text(self) -> str:
        ...
        try:
            ...
            if self.validate_format and not self._validate_format(text):
                error_message = i18n.t(
                    "components.input_output.text.errors.format_validation_failed", format=self.text_type
                )
                self.status = error_message
                raise ValueError(error_message)

            success_message = i18n.t("components.input_output.text.success.text_processed", length=len(text))
            self.status = success_message
            return text

        except Exception as e:
            error_message = i18n.t("components.input_output.text.errors.text_processing_error", error=str(e))
            self.status = error_message
        ...
        try:
            ...
            if self.text_type == "template" and self.template_variables:
                try:
                    ...
                    data_dict["template_variables"] = json.loads(self.template_variables)
            ...
            error_message = i18n.t("components.input_output.text.errors.data_creation_error", error=str(e))
            self.status = error_message
            raise ValueError(error_message) from e
    ...
    def get_processed_text(self) -> str:
        ...
        try:
            ...
            success_message = i18n.t("components.input_output.text.success.processed_text_created", length=len(text))
            self.status = success_message
            return text

        except Exception as e:
            error_message = i18n.t("components.input_output.text.errors.processed_text_error", error=str(e))
            self.status = error_message
            raise ValueError(error_message) from e
    ...
    def get_text_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            error_message = i18n.t("components.input_output.text.errors.text_info_error", error=str(e))
            return {"error": str(e)}
