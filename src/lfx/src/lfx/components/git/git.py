...
class GitLoaderComponent(Component):
    ...
    inputs = [
        ...
        MessageTextInput(
            name="content_filter",
            display_name=i18n.t("components.git.git.content_filter.display_name"),
            required=False,
            advanced=True,
    ...
    outputs = [
        Output(
            name="data", display_name=i18n.t("components.git.git.outputs.data.display_name"), method="load_documents"
        ),
    ]
    ...
    def is_binary(file_path: str | Path) -> bool:
        ...
        try:
            with Path(file_path).open("rb") as file:
                ...
                if is_bin:
                    logger.debug(i18n.t("components.git.git.logs.binary_file_detected", path=str(file_path)))
                return is_bin
        except Exception as e:
            logger.debug(i18n.t("components.git.git.logs.binary_check_failed", path=str(file_path), error=str(e)))
            return True
    ...
    def check_file_patterns(file_path: str | Path, patterns: str) -> bool:
        ...
        for pattern in pattern_list:
            if pattern.startswith("!"):
                ...
                if fnmatch(path_str, exclude_pattern) or fnmatch(file_name, exclude_pattern):
                    logger.debug(
                        i18n.t("components.git.git.logs.file_excluded", path=path_str, pattern=exclude_pattern)
                    )
                    return False
        ...
        matched = any(fnmatch(path_str, pattern) or fnmatch(file_name, pattern) for pattern in include_patterns)
        if matched:
            logger.debug(i18n.t("components.git.git.logs.file_included", path=path_str))
        return matched
    ...
    def check_content_pattern(file_path: str | Path, pattern: str) -> bool:
        ...
        try:
            ...
            with Path(file_path).open("rb") as file:
                content = file.read(1024)
                if b"\x00" in content:
                    logger.debug(i18n.t("components.git.git.logs.content_check_skipped_binary", path=str(file_path)))
                    return False
            ...
            try:
                ...
                logger.warning(i18n.t("components.git.git.logs.invalid_regex", pattern=pattern, error=str(e)))
                return False
            ...
            if matched:
                logger.debug(i18n.t("components.git.git.logs.content_matched", path=str(file_path)))
            return matched
        except (OSError, UnicodeDecodeError) as e:
            logger.debug(i18n.t("components.git.git.logs.content_check_failed", path=str(file_path), error=str(e)))
            return False
    ...
    def build_combined_filter(self, file_filter_patterns: str | None = None, content_filter_pattern: str | None = None):
        ...
        logger.debug(
            i18n.t(
                "components.git.git.logs.building_filter",
        ...
        def combined_filter(file_path: str) -> bool:
            try:
                ...
                if not path.exists():
                    logger.debug(i18n.t("components.git.git.logs.file_not_exists", path=str(path)))
                    return False
                ...
                logger.debug(i18n.t("components.git.git.logs.filter_error", path=file_path, error=str(e)))
                return False
    ...
    async def temp_clone_dir(self):
        ...
        try:
            ...
            logger.debug(i18n.t("components.git.git.logs.temp_dir_created", path=temp_dir))
            yield temp_dir
        finally:
            if temp_dir:
                try:
                    await anyio.Path(temp_dir).rmdir()
                    logger.debug(i18n.t("components.git.git.logs.temp_dir_removed", path=temp_dir))
                except Exception as e:
                    logger.warning(
    ...
    def update_build_config(self, build_config: dict, field_value: str, field_name: str | None = None, action: str | None = None) -> dict:
        ...
        if field_name == "repo_source":
            if field_value == local_label:
                ...
                logger.debug(i18n.t("components.git.git.logs.config_updated_local"))
                ...
                logger.debug(i18n.t("components.git.git.logs.config_updated_remote"))
    ...
    async def build_gitloader(self) -> GitLoader:
        ...
        if repo_source == local_label:
            ...
            clone_url = None
            logger.info(i18n.t("components.git.git.logs.using_local_repo", path=repo_path))
        else:
            # Clone source
            clone_url = self.clone_url
            logger.info(i18n.t("components.git.git.logs.using_remote_repo", url=clone_url))
        ...
        if not branch:
            ...
            logger.debug(i18n.t("components.git.git.logs.using_branch", branch=branch))
        ...
        try:
            ...
            error_msg = i18n.t("components.git.git.errors.gitloader_creation_failed", error=str(e))
    ...
    async def load_documents(self) -> list[Data]:
        ...
        try:
            ...
            logger.info(i18n.t("components.git.git.logs.documents_loaded", count=len(data)))

            self.status = data
            return data

        except Exception as e:
            error_msg = i18n.t("components.git.git.errors.loading_failed", error=str(e))
            logger.exception(error_msg)
            raise ValueError(error_msg) from e
