...
class YfinanceToolComponent(LCToolComponent):
    ...
    inputs = [
        MessageTextInput(
            name="symbol",
            display_name=i18n.t("components.tools.yahoo_finance.symbol.display_name"),
            info=i18n.t("components.tools.yahoo_finance.symbol.info"),
        ),
        DropdownInput(
            name="method",
            display_name=i18n.t("components.tools.yahoo_finance.method.display_name"),
        ...
        IntInput(
            name="num_news",
            display_name=i18n.t("components.tools.yahoo_finance.num_news.display_name"),
    ...
    def run_model(self) -> list[Data]:
        try:
            if not self.symbol or not self.symbol.strip():
                warning_message = i18n.t("components.tools.yahoo_finance.warnings.empty_symbol")
                self.status = warning_message
                return [Data(data={"error": warning_message})]

            executing_message = i18n.t(
                "components.tools.yahoo_finance.info.retrieving_data", symbol=self.symbol, method=self.method.value
            )
            ...
            error_message = i18n.t("components.tools.yahoo_finance.errors.execution_failed", error=str(e))
    ...
    def build_tool(self) -> Tool:
        try:
            tool = StructuredTool.from_function(
                name="yahoo_finance",
                description=i18n.t("components.tools.yahoo_finance.tool_description"),
                func=self._yahoo_finance_tool,
                args_schema=YahooFinanceSchema,
            )

            success_message = i18n.t("components.tools.yahoo_finance.success.tool_created")
            self.status = success_message
            return tool

        except Exception as e:
            error_message = i18n.t("components.tools.yahoo_finance.errors.tool_creation_failed", error=str(e))
            self.status = error_message
    ...
    def _yahoo_finance_tool(
        ...
        try:
            ...
            error_message = i18n.t("components.tools.yahoo_finance.errors.import_error")
            raise ImportError(error_message) from e

        if not symbol or not symbol.strip():
            error_message = i18n.t("components.tools.yahoo_finance.errors.empty_symbol")
        ...
        try:
            ...
            if isinstance(method, str):
                try:
                    ...
                    error_message = i18n.t("components.tools.yahoo_finance.errors.invalid_method", method=method)
                    raise ToolException(error_message)

            if method == YahooFinanceMethod.GET_INFO:
                result = ticker.info
                if not result:
                    error_message = i18n.t("components.tools.yahoo_finance.errors.no_info_data", symbol=symbol)
                    raise ToolException(error_message)
            elif method == YahooFinanceMethod.GET_NEWS:
                result = ticker.news
                if not result:
                    error_message = i18n.t("components.tools.yahoo_finance.errors.no_news_data", symbol=symbol)
                    raise ToolException(error_message)
                ...
                try:
                    ...
                    if result is None or (hasattr(result, "empty") and result.empty):
                        error_message = i18n.t(
                            "components.tools.yahoo_finance.errors.no_data_available",
                            method=method.value,
                            symbol=symbol,
                        )
                        raise ToolException(error_message)
            ...
            if method == YahooFinanceMethod.GET_NEWS:
                if isinstance(result, str):
                    try:
                        ...
                        data_list = [Data(data=article) for article in parsed_result]
                    except (ValueError, SyntaxError):
                        # If parsing fails, return as single data item
                        data_list = [Data(data={"news": result, "symbol": symbol})]
                else:
                    # If result is already a list/dict
                    data_list = [Data(data=article) for article in result]
            else:
                data_list = [Data(data={"result": formatted_result, "symbol": symbol, "method": method.value})]

            success_message = i18n.t(
                "components.tools.yahoo_finance.success.data_retrieved",
                method=method.value,
                symbol=symbol,
                count=len(data_list),
            )
            ...
            error_message = i18n.t(
                "components.tools.yahoo_finance.errors.data_retrieval_failed",
                method=method.value,
                symbol=symbol,
                error=str(e),
            )
