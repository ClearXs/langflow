...
class SearXNGToolComponent(LCToolComponent):
    ...
    inputs = [
        ...
        IntInput(
            name="max_results",
            display_name=i18n.t("components.tools.searxng.max_results.display_name"),
        ...
        MultiselectInput(
            name="categories",
            display_name=i18n.t("components.tools.searxng.categories.display_name"),
        ...
        DropdownInput(
            name="language",
            display_name=i18n.t("components.tools.searxng.language.display_name"),
    ...
    outputs = [
        Output(
            display_name=i18n.t("components.tools.searxng.outputs.result_tool.display_name"),
            name="result_tool",
            method="build_tool",
        ),
    ]
    ...
    def update_build_config(
        ...
        try:
            ...
            success_message = i18n.t(
                "components.tools.searxng.success.config_loaded",
            ...
            error_message = i18n.t("components.tools.searxng.errors.connection_failed", url=field_value, error=str(e))
            self.status = error_message
            logger.debug(self.status, exc_info=True)
            build_config["categories"]["options"] = [i18n.t("components.tools.searxng.errors.connection_error")]
        except Exception as e:  # noqa: BLE001
            error_message = i18n.t("components.tools.searxng.errors.config_parsing_failed", error=str(e))
            self.status = error_message
            logger.debug(self.status, exc_info=True)
            build_config["categories"]["options"] = [i18n.t("components.tools.searxng.errors.failed_to_parse"), str(e)]
        return build_config
            ...
            def search(query: str, categories: Sequence[str] = ()) -> list:
                try:
                    if not query or not query.strip():
                        warning_message = i18n.t("components.tools.searxng.warnings.empty_query")
                        return [{"error": warning_message}]

                    if not SearxSearch._categories and not categories:
                        error_message = i18n.t("components.tools.searxng.errors.no_categories")
                    ...
                    if response.status_code != 200:
                        error_message = i18n.t(
                            "components.tools.searxng.errors.search_request_failed", status=response.status_code
                        )
                    ...
                    if not results:
                        warning_message = i18n.t("components.tools.searxng.warnings.no_results", query=query)
                    ...
                    error_message = i18n.t("components.tools.searxng.errors.search_connection_failed", error=str(e))
                    logger.debug("Error running SearXNG Search", exc_info=True)
                    return [{"error": error_message}]
                except Exception as e:  # noqa: BLE001
                    error_message = i18n.t("components.tools.searxng.errors.search_failed", error=str(e))
                    logger.debug("Error running SearXNG Search", exc_info=True)
                    return [{"error": error_message}]

        try:
            if not self.url:
                error_message = i18n.t("components.tools.searxng.errors.url_required")
                self.status = error_message
                raise ValueError(error_message)
            ...
            schema_fields = {
                ...
                "categories": (
                    list[str],
                    Field(default=[], description=i18n.t("components.tools.searxng.schema.categories.description")),
                ),
            }
            ...
            available_categories = (
                ...
                if self.categories
                else i18n.t("components.tools.searxng.no_categories_configured")
            )
            tool_description = i18n.t("components.tools.searxng.tool_description", categories=available_categories)

            success_message = i18n.t(
                "components.tools.searxng.success.tool_created", url=self.url, max_results=self.max_results
            )
            self.status = success_message
            ...
            error_message = i18n.t("components.tools.searxng.errors.tool_creation_failed", error=str(e))
            self.status = error_message
