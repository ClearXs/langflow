...
from lfx.io import DataInput, MultilineInput, BoolInput, Output
...
class ETLPythonScriptComponent(Component):
    ...
    inputs = [
        DataInput(name="data_input", display_name=i18n.t('components.scripts.python_script.data_input.display_name'), info=i18n.t('components.scripts.python_script.data_input.info'), is_list=True),
        MultilineInput(name="script", display_name=i18n.t('components.scripts.python_script.script.display_name'), info=i18n.t('components.scripts.python_script.script.info'), required=True),
        BoolInput(name="capture_output", display_name=i18n.t('components.scripts.python_script.capture_output.display_name'), info=i18n.t('components.scripts.python_script.capture_output.info'), value=True, advanced=True)
    ]
    ...
    def execute_python(self) -> Data:
        try:
            ...
            output_data = {"result": local_vars.get("result"), "stdout": stdout_capture.getvalue() if stdout_capture else "", "stderr": stderr_capture.getvalue() if stderr_capture else "", "success": True}
