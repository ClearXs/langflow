...
class ETLDeduplicationComponent(Component):
    ...
    def deduplicate(self) -> list[Data]:
        ...
        try:
            ...
            self.status = i18n.t('components.operations.deduplication.status.success',
                                 unique=len(result_data), duplicates=removed_count)
    ...
    def get_duplicates(self) -> list[Data]:
        ...
        # Mark duplicates
        duplicates_mask = df.duplicated(subset=subset_cols, keep=self.keep_strategy if self.keep_strategy != "none" else False)
