...
export BACKEND_URL FRONTEND_PORT LANGFLOW_MAX_FILE_SIZE_UPLOAD DATA_API_URL

# Use envsubst to substitute environment variables in the template
envsubst '${BACKEND_URL} ${FRONTEND_PORT} ${LANGFLOW_MAX_FILE_SIZE_UPLOAD} ${DATA_API_URL}' < /etc/nginx/conf.d/default.conf.template > $CONFIG_DIR/default.conf
